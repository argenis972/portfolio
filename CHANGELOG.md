...
#### Added
- **Minimum Viable IaC**: Introduced Terraform provisioning in `infrastructure/` focused on the Koyeb backend service. Purposefully scoped to avoid over-engineering external stateful services. See ADR-19.
- **Chaos E2E CI Testing**: Added a weekly GitHub Actions workflow that spins up a local `docker-compose` environment and runs narrative E2E tests against the Chaos Playground endpoints. Ensures the system actually degrades and recovers as advertised. See ADR-18.
