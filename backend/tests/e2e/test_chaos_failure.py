...
def test_chaos_failure(api_client, chaos_teardown):
    ...
    assert health_data.get("status") in ("degraded", "ok", "error"), "Health status is missing"
    ...
    assert metrics.get("system_lifecycle") in ("DEGRADED", "RECOVERING"), "System status should reflect failure"
